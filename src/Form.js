            ...
            <form id="form" className="form-horizontal">
                <div className="form-group">
                    ...
                    <div className="col-8 col-sm-12 d-flex">
                        ...
                        <span className="ml-2 text-nowrap">руб.</span>
                    </div>
                </div>
                ...
                <div className="form-group">
                    ...
                    <div className="col-8 col-sm-12 d-flex">
                        ...
                        <select name="periodType" className="form-select ml-2" value={credit.periodType} onChange={this.onChange}>
                ...
                <div className="form-group">
                    ...
                    <div className="col-8 col-sm-12 d-flex">
                        <select name="floatRate" className="form-select" value={credit.floatRate} onChange={this.onChange}>
                            <option value="constant">Постоянная</option>
                            <option value="variable">Изменяемая</option>
                        </select>
                        <input required type="number" min="0" step="0.1" name="percent" id="percent" className="form-input ml-2" value={credit.percent} onChange={this.onChange}/>
                        ...
                        {credit.payments.map((payment, i) =>
                            <div key={payment.key} className="payment columns">
                                ...
                                <div className="column col-6 d-flex">
                                    <select name={`payment.${i}.recalc`} className="form-select" value={payment.recalc} onChange={this.changePayment}>
                                ...
                                <div className="column col-6 mt-2">
                                    <input type="date" name={`payment.${i}.date`} className="form-input" value={payment.date} onChange={this.changePayment}/>
                                </div>
                                <div className="column col-6 mt-2">
                                    <select name={`payment.${i}.next`} className="form-select" value={payment.next} onChange={this.changePayment}>
                                        <option value="0">Следующий платеж - без изменений</option>
                                        <option value="1">Следующий платеж - только проценты</option>
                                    </select>
                                </div>
                                <div className="column col-6 mt-2">
                                    <input type="number" name={`payment.${i}.sum`} className="form-input" placeholder="Сумма" value={payment.sum} onChange={this.changePayment}/>
                                </div>
                            </div>
                        )}
                        <button type="button" id="add-button" className="btn btn-link" onClick={this.addPayment}><i className="icon icon-plus"/> Добавить досрочный платеж</button>
                    </div>
                </div>
